...
public class ContainerSilverAnvil extends Container {
    ...
    public ContainerSilverAnvil(InventoryPlayer playerInventory, World worldIn, EntityPlayer player) {
        this(playerInventory, worldIn, BlockPos.ORIGIN, player);
    }

    public ContainerSilverAnvil(InventoryPlayer playerInventory, final World worldIn, final BlockPos blockPosIn, EntityPlayer player) {
        this.maximumCap = 160;
        this.outputSlot = new InventoryCraftResult();
        this.inputSlots = new InventoryBasic("Repair", true, 2) {
            public void markDirty() {
                super.markDirty();
                ContainerSilverAnvil.this.onCraftMatrixChanged(this);
            }
        };
        ...
        this.addSlotToContainer(new Slot(this.outputSlot, 2, 134, 47) {
            ...
            public boolean canTakeStack(EntityPlayer playerIn) {
                return (playerIn.capabilities.isCreativeMode || playerIn.experienceLevel >= ContainerSilverAnvil.this.maximumCost) && ContainerSilverAnvil.this.maximumCost > 0 && this.getHasStack();
            }

            @Nonnull
            public ItemStack onTake(EntityPlayer playerIn, @Nonnull ItemStack stack) {
                if (!playerIn.capabilities.isCreativeMode) {
                    playerIn.addExperienceLevel(-ContainerSilverAnvil.this.maximumCost);
                }

                float breakChance = this.getBreakChance(playerIn, stack);
                ContainerSilverAnvil.this.inputSlots.setInventorySlotContents(0, ItemStack.EMPTY);
                if (ContainerSilverAnvil.this.materialCost > 0) {
                    ItemStack itemstack = ContainerSilverAnvil.this.inputSlots.getStackInSlot(1);
                    if (!itemstack.isEmpty() && itemstack.getCount() > ContainerSilverAnvil.this.materialCost) {
                        itemstack.shrink(ContainerSilverAnvil.this.materialCost);
                        ContainerSilverAnvil.this.inputSlots.setInventorySlotContents(1, itemstack);
                    } else {
                        ContainerSilverAnvil.this.inputSlots.setInventorySlotContents(1, ItemStack.EMPTY);
                    }
                } else {
                    ContainerSilverAnvil.this.inputSlots.setInventorySlotContents(1, ItemStack.EMPTY);
                }

                ContainerSilverAnvil.this.maximumCost = 0;
                IBlockState iblockstate = worldIn.getBlockState(blockPosIn);
                if (!playerIn.capabilities.isCreativeMode && !worldIn.isRemote && iblockstate.getBlock() instanceof BlockSilverAnvil && playerIn.getRNG().nextFloat() < breakChance) {
                    iblockstate = BlockSilverAnvil.damage(iblockstate);
                    if (iblockstate == null) {
            ...
            public float getBreakChance(EntityPlayer playerIn, ItemStack stack) {
                return ForgeHooks.onAnvilRepair(playerIn, stack, ContainerSilverAnvil.this.inputSlots.getStackInSlot(0), ContainerSilverAnvil.this.inputSlots.getStackInSlot(1)) / 64.0F;
            }
        });
    ...
    public boolean canInteractWith(@Nonnull EntityPlayer playerIn) {
        return this.theWorld.getBlockState(this.selfPosition).getBlock() instanceof BlockSilverAnvil && playerIn.getDistanceSq((double)this.selfPosition.getX() + 0.5D, (double)this.selfPosition.getY() + 0.5D, (double)this.selfPosition.getZ() + 0.5D) <= 64.0D;
    }
    ...
    public static boolean onAnvilChange(ContainerSilverAnvil container, ItemStack left, ItemStack right, IInventory outputSlot, String name, int baseCost) {
