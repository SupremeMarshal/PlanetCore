...
public class OnyxBowV extends ItemBow
{
    private Item arrow;
    private int damage;
    public OnyxBowV()
    {
        ...
        this.addPropertyOverride(new ResourceLocation("pulling_onyx_v"), new IItemPropertyGetter()
        {
            @SideOnly(Side.CLIENT)
            public float apply(ItemStack stack, @Nullable World worldIn, @Nullable EntityLivingBase entityIn)
            {
                return entityIn != null && arrow == ModItems.ONYX_ARROW_V && entityIn.isHandActive() && entityIn.getActiveItemStack() == stack ? 1.0F : 0.0F;
            }
        });
    }
    ...
    protected boolean isArrow(ItemStack stack)
    {
        if(stack.getItem() instanceof ItemArrow
                ...
                || stack.getItem() == ModItems.ONYX_ARROW_V)
        {
            return true;
        }
        return false;
            ...
            if (!itemstack.isEmpty() || flag)
            {
                ...
                if ((double)f >= 0.1D)
                {
                    ...
                    if (!worldIn.isRemote) {
                        ...
                        if (itemstack.getItem() == ModItems.ONYX_ARROW_V) {
                            entityarrow = ((OnyxArrowV) itemarrow).createArrow(worldIn, itemstack, entityplayer);
                            damage = 32;
                        }

                        entityarrow = this.customizeArrow(entityarrow);
                        entityarrow.shoot(entityplayer, entityplayer.rotationPitch, entityplayer.rotationYaw, 0.0F, f * 3.0F, 1.0F);
                        entityarrow.setDamage(damage + 18);
                        ...
                        if (j > 0) {
                            entityarrow.setDamage(damage + (double) j * 0.5D + 18.5D);
                        }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        tooltip.add(net.minecraft.client.resources.I18n.format("Can use up to onyx V tier arrows."));
        tooltip.add(net.minecraft.client.resources.I18n.format("Increased damage: +18"));
