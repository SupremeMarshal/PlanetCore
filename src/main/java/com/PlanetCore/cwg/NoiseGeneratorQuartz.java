...
public class NoiseGeneratorQuartz implements ICubicStructureGenerator {
    //V17
    private IBuilder builder;

    public NoiseGeneratorQuartz() {
    }
    ...
    public void generate(World world, CubePrimer cubePrimer, CubePos cubePos) {
        ...
        builder.forEachScaled(start, end, new Vec3i(4, 4, 4), (x, y, z, gradX, gradY, gradZ, value) -> {
            ...
            if (!skip) {
                if (value > 0) {
                    cubePrimer.setBlockState(x, y, z, ModBlocks.ROCK.getStateFromMeta(0));
                } else {
                    if (value + gradY > 0) {
