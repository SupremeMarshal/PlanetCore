...
public class NoiseGeneratorCorestone implements ICubicStructureGenerator {
    //V17
    private IBuilder builder;
    private IBlockState stoneBlock;

    public NoiseGeneratorCorestone() {
    }
    ...
    public void generate(World world, CubePrimer cubePrimer, CubePos cubePos) {
        if (this.builder == null) {
            ...
            IBuilder caveNoise = NoiseSource.perlin().frequency(caveScale).octaves(caveOctaves).normalizeTo(-1, 1).seed(world.getSeed() + 1032).create()
                    .add((x, y, z) -> y > yCutoff ? ((yCutoff - y) * yCutoffFactor1) : y < yCutoff1 ? ((yCutoff1 - y) * yCutoffFactor3) : 0);

            IBuilder placementNoise = NoiseSource.perlin().frequency(placementScale).octaves(placementOctaves).normalizeTo(-1, 1).seed(world.getSeed() + 1033).create()
        ...
        builder.forEachScaled(start, end, new Vec3i(4, 4, 4), (x, y, z, gradX, gradY, gradZ, value) -> {
            ...
            if (value > 0 && cubePrimer.getBlockState(x,y,z) != Blocks.AIR) {
                cubePrimer.setBlockState(x, y, z, ModBlocks.ROCK.getStateFromMeta(4));
            } else {
                if (value + gradY > 0) {
