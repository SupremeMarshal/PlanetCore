...
public class NoiseGeneratorGarnet implements ICubicStructureGenerator {
    //V17
    private IBuilder builder;

    public NoiseGeneratorGarnet() {
    }
    ...
    public void generate(World world, CubePrimer cubePrimer, CubePos cubePos) {
        if (this.builder == null) {
            ...
            IBuilder caveNoise = NoiseSource.perlin().frequency(caveScale).octaves(caveOctaves).normalizeTo(-1, 1).seed(world.getSeed() + 1028).create()
                    .add((x, y, z) -> y > yCutoff ? ((yCutoff - y) * yCutoffFactor1) : y < yCutoff1 ? ((yCutoff1 - y) * yCutoffFactor3) : 0);

            IBuilder placementNoise = NoiseSource.perlin().frequency(placementScale).octaves(placementOctaves).normalizeTo(-1, 1).seed(world.getSeed() + 1029).create()
        ...
        builder.forEachScaled(start, end, new Vec3i(4, 4, 4), (x, y, z, gradX, gradY, gradZ, value) -> {
            ...
            if (!skip) {
                if (value > 0) {
                    cubePrimer.setBlockState(x, y, z, ModBlocks.ROCK.getStateFromMeta(2));
                }
                else {
                    if (value + gradY > 0) {
