...
public class CellNoiseRareOreCaveGenerator implements ICubicStructureGenerator {
    private final long seedModifier;
    private final double cellSizeXZ = 32;
    private final double cellSizeY = 32;
    private final double cellFreqXZ = 1 / cellSizeXZ;
    private final double cellFreqY = 1 / cellSizeY;
    private final double cellTurbulenceFreqXZ = 1.0 / 32;
    private final double cellTurbulenceFreqY = 1.0 / 32;
    private final double cellTurbulenceAmp = 1;
    public CellNoiseRareOreCaveGenerator(long seedModifier) {
    ...
    private CellCaveType caveTypeForCell(int cellX, int cellY, int cellZ, int cell, FastRandom rand) {
        if (cellY < 0 && rand.nextBoolean()) {
            if (rand.nextFloat() < 0.02 && cellY < -250) {
                return CellCaveTypes.SILVER_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -500) {
                return CellCaveTypes.GOLD_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -2000) {
                return CellCaveTypes.DIAMOND_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -2500) {
                return CellCaveTypes.EMERALD_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -3000) {
                return CellCaveTypes.RUBY_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -3500) {
                return CellCaveTypes.SAPPHIRE_CAVE;
            }
            else if (rand.nextFloat() < 0.005 && cellY < -2000) {
                return CellCaveTypes.LAPIS_CAVE;
            }
            else if (rand.nextFloat() < 0.005 && cellY < -2000) {
                return CellCaveTypes.REDSTONE_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -4000) {
                return CellCaveTypes.MAJORITE_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -5000) {
                return CellCaveTypes.AMAZONITE_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -6000) {
                return CellCaveTypes.ONYX_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -6300) {
                return CellCaveTypes.PAINITE_CAVE;
            }
    ...
    private Supplier<IBlockState> lavaBlock(int cellX, int cellY, int cellZ, CellCaveType type, int cell, FastRandom rand) {
       if (type == CellCaveTypes.COAL_CAVE) return () -> Blocks.LAVA.getDefaultState();
       else if (type == CellCaveTypes.IRON_CAVE) return () -> ModBlocks.IRON_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.SULFUR_CAVE) return () -> Blocks.LAVA.getDefaultState();
       else if (type == CellCaveTypes.SILVER_CAVE) return () -> ModBlocks.SILVER_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.GOLD_CAVE) return () -> ModBlocks.GOLD_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.DIAMOND_CAVE) return () -> ModBlocks.DIAMOND_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.REDSTONE_CAVE) return () -> ModBlocks.REDSTONE_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.EMERALD_CAVE) return () -> Blocks.LAVA.getDefaultState();
       else if (type == CellCaveTypes.LAPIS_CAVE) return () -> Blocks.LAVA.getDefaultState();
       else if (type == CellCaveTypes.TITANIUM_CAVE) return () -> ModBlocks.TITANIUM_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.URANIUM_CAVE) return () -> ModBlocks.URANIUM_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.TUNGSTEN_CAVE) return () -> ModBlocks.TUNGSTEN_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.RUBY_CAVE) return () -> ModBlocks.RUBY_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.SAPPHIRE_CAVE) return () -> ModBlocks.SAPPHIRE_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.MAJORITE_CAVE) return () -> ModBlocks.MAJORITE_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.AMAZONITE_CAVE) return () -> ModBlocks.AMAZONITE_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.ONYX_CAVE) return () -> ModBlocks.ONYX_LAVA_FLUID.getDefaultState();
       else if (type == CellCaveTypes.PAINITE_CAVE) return () -> ModBlocks.PAINITE_LAVA_FLUID.getDefaultState();
       else return () -> Blocks.LAVA.getDefaultState();
    }

    @SuppressWarnings("Convert2MethodRef")
    private Supplier<IBlockState> airBlock(int cellX, int cellY, int cellZ, CellCaveType type, int cell, FastRandom rand) {
        if (type == CellCaveTypes.COAL_CAVE) return () -> Blocks.LAVA.getDefaultState();
        else if (type == CellCaveTypes.IRON_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_IRON.getDefaultState();
        else if (type == CellCaveTypes.SULFUR_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_SULFUR.getDefaultState();
        else if (type == CellCaveTypes.SILVER_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_SILVER.getDefaultState();
        else if (type == CellCaveTypes.GOLD_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_GOLD.getDefaultState();
        else if (type == CellCaveTypes.DIAMOND_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_DIAMOND.getDefaultState();
        else if (type == CellCaveTypes.REDSTONE_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_REDSTONE.getDefaultState();
        else if (type == CellCaveTypes.EMERALD_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_EMERALD.getDefaultState();
        else if (type == CellCaveTypes.LAPIS_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_LAPIS.getDefaultState();
        else if (type == CellCaveTypes.TITANIUM_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_TITANIUM.getDefaultState();
        else if (type == CellCaveTypes.URANIUM_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_URANIUM.getDefaultState();
        else if (type == CellCaveTypes.TUNGSTEN_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_TUNGSTEN.getDefaultState();
        else if (type == CellCaveTypes.RUBY_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_RUBY.getDefaultState();
        else if (type == CellCaveTypes.SAPPHIRE_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_SAPPHIRE.getDefaultState();
        else if (type == CellCaveTypes.MAJORITE_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_MAJORITE.getDefaultState();
        else if (type == CellCaveTypes.AMAZONITE_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_AMAZONITE.getDefaultState();
        else if (type == CellCaveTypes.ONYX_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_ONYX.getDefaultState();
        else if (type == CellCaveTypes.PAINITE_CAVE) return () -> ModBlocks.SUPERCOMPRESSED_PAINITE.getDefaultState();
    ...
    @Override public void generate(World world, CubePrimer cubePrimer, CubePos cubePos) {
        ...
        generateScaled(worldSeed ^ seedModifier, start, end, new Vec3i(4, 4, 4),
                (x, y, z, value, lavaY, lavaBlock, airBlock, cell, cellType, cellX, cellY, cellZ) -> {
                    ...
                    if (value > 0 && cubePrimer.getBlockState(x,y,z) != Blocks.AIR.getDefaultState()) {
                    ...
                    if (value > 0.2)
                    {
                        if (blockY <= lavaY) {
                            boolean skip = false;
                            if (cubePrimer.getBlockState(x, y, z).getBlock() == Blocks.AIR) {
                                skip = true;
                            }
                            if (!skip) {
