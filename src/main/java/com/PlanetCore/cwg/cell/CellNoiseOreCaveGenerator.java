...
public class CellNoiseOreCaveGenerator implements ICubicStructureGenerator {
    private final long seedModifier;
    private final double cellSizeXZ = 64;
    private final double cellSizeY = 64;
    private final double cellFreqXZ = 1 / cellSizeXZ;
    private final double cellFreqY = 1 / cellSizeY;
    private final double cellTurbulenceFreqXZ = 1.0 / 128;
    private final double cellTurbulenceFreqY = 1.0 / 128;
    private final double cellTurbulenceAmp = 1;
    public CellNoiseOreCaveGenerator(long seedModifier) {
    ...
    private CellCaveType caveTypeForCell(int cellX, int cellY, int cellZ, int cell, FastRandom rand) {
        if (cellY < 0) {
            if (rand.nextFloat() < 0.02 && cellY > -250) {
                ...
                return CellCaveTypes.IRON_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY > -1000) {
                return CellCaveTypes.SULFUR_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -500 && cellY > -2500) {
                return CellCaveTypes.TITANIUM_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -750 && cellY > -2500) {
                return CellCaveTypes.URANIUM_CAVE;
            }
            else if (rand.nextFloat() < 0.02 && cellY < -750 && cellY > -3000) {
                return CellCaveTypes.TUNGSTEN_CAVE;
            }
