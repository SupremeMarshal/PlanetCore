...
public class CellNoiseRockBiomeGenerator implements ICubicStructureGenerator {
    ...
    private final double cellTurbulenceFreqXZ = 1.0 / 64;
    private final double cellTurbulenceFreqY = 1.0 / 64;
    private final double cellTurbulenceAmp = 1;
    public CellNoiseRockBiomeGenerator(long seedModifier) {
    ...
    private CellCaveType caveTypeForCell(int cellX, int cellY, int cellZ, int cell, FastRandom rand) {
        if (cellY < 0 && rand.nextFloat() < 0.25) {
            if (rand.nextFloat() < 0.66 && cellY < 0 && cellY > -512) {
                if (rand.nextBoolean()) {
                    return CellCaveTypes.QUARTZ_CAVE;
                }
                else return CellCaveTypes.GRANITE_CAVE;
            }
            else if (rand.nextFloat() < 0.66 && cellY < -512 && cellY > -1000) {
                if (rand.nextBoolean()) {
                    return CellCaveTypes.BLACKSTONE_CAVE;
                }
                else return CellCaveTypes.PERIDOTITE_CAVE;
            }
            else if (rand.nextFloat() < 0.66 && cellY < -1000 && cellY > -1920) {
                if (rand.nextBoolean()) {
                    return CellCaveTypes.GARNET_CAVE;
                }
                else return CellCaveTypes.PERIDOTITE_CAVE;
            }
            else if (rand.nextFloat() < 0.66 && cellY < -1920 && cellY > -3200) {
                if (rand.nextBoolean()) {
                    return CellCaveTypes.BRIDGMANITE_CAVE;
                }
                else return CellCaveTypes.FERROPERICLASE_CAVE;
            }
            else if (rand.nextFloat() < 0.66 && cellY < -3200 && cellY > -5000) {
                if (rand.nextBoolean()) {
                    return CellCaveTypes.CORESTONE_CAVE;
                }
                else return CellCaveTypes.FERROPERICLASE_CAVE;
            }
            else if (cellY < -5000)
            {
                ...
                else return CellCaveTypes.ONYXSLATE_CAVE;
            }
    ...
    private Supplier<IBlockState> lavaBlock(int cellX, int cellY, int cellZ, CellCaveType type, int cell, FastRandom rand) {
       if (type == CellCaveTypes.CORESTONE_CAVE) return () -> ModBlocks.SUPERHEATED_LAVA_FLUID.getDefaultState();
       else return () -> ModBlocks.IRON_LAVA_FLUID.getDefaultState();
    }

    @SuppressWarnings("Convert2MethodRef")
    private Supplier<IBlockState> airBlock(int cellX, int cellY, int cellZ, CellCaveType type, int cell, FastRandom rand) {

        if (type == CellCaveTypes.QUARTZ_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(0);
        else if (type == CellCaveTypes.GRANITE_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(1);
        else if (type == CellCaveTypes.BLACKSTONE_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(2);
        else if (type == CellCaveTypes.PERIDOTITE_CAVE) {
            if (cellY > -1500) return () -> ModBlocks.ROCK.getStateFromMeta(3);
            else return () -> ModBlocks.ROCK.getStateFromMeta(4);
    }
        else if (type == CellCaveTypes.GARNET_CAVE) {
            if (cellY > -1500) return () -> ModBlocks.ROCK.getStateFromMeta(5);
            else return () -> ModBlocks.ROCK.getStateFromMeta(6);
        }
        else if (type == CellCaveTypes.BRIDGMANITE_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(8);
        else if (type == CellCaveTypes.FERROPERICLASE_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(10);
        else if (type == CellCaveTypes.CORESTONE_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(11);
        else if (type == CellCaveTypes.ONYXSLATE_CAVE) return () -> ModBlocks.ROCK.getStateFromMeta(13);
